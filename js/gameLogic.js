...
const START_GOLD = 10;
const BASE_HP    = 12; // совпадает с core.js
...
export let turn      = 1;
export let current   = 1; // 1 – человек, 2 – ИИ
...
export function endTurnBtn() {
  askYesNo('Закончить ход?', endTurn);
}
...
function resetState() {
  ...
  units = [
    { id: 1, owner: 1, type: 'INF', r: 2,        c: 1,       hp: UNITS.INF.hp, movesLeft: UNITS.INF.move },
    { id: 2, owner: 1, type: 'CAV', r: 2,        c: 2,       hp: UNITS.CAV.hp, movesLeft: UNITS.CAV.move },
    { id: 3, owner: 2, type: 'INF', r: ROWS - 3, c: COLS - 2, hp: UNITS.INF.hp, movesLeft: UNITS.INF.move },
    { id: 4, owner: 2, type: 'ART', r: ROWS - 4, c: COLS - 3, hp: UNITS.ART.hp, movesLeft: UNITS.ART.move },
  ];
...
function aiTurn() {
  units.filter(u => u.owner === 2).forEach(me => {
    ...
    const dist = Math.abs(me.r - target.r) + Math.abs(me.c - target.c);
...
function nearestEnemyOrBase(me) {
  ...
  return list.reduce((best, cur) => {
    const d = Math.abs(me.r - cur.r) + Math.abs(me.c - cur.c);
...
function bestStepTowards(me, target, steps) {
  ...
  for (const [dr, dc] of dirs.sort(() => Math.random() - 0.5)) {
    ...
    if (Math.abs(nr - target.r) + Math.abs(nc - target.c) < Math.abs(me.r - target.r) + Math.abs(me.c - target.c)) {
      return { r: nr, c: nc };
    }
  }
  return null;
